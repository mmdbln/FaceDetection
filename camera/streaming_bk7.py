...
#**************************************************************************************************************************
        ...
        try:
            ...
            for face_obj in face_objs:
                ...
                if facial_area["w"] <= 50:  # discard small detected faces
                    continue
                faces.append(
                    (
                        facial_area["x"],
            ...
            # # directory of our database
                ...
                for tem_img in os.listdir(temporary_path):
                    ...
                    for image in os.listdir(database_path):
                        ...
                        verifying = DeepFace.verify(img1_path = tem_img_, img2_path = db_img, enforce_detection = False)
                        result = verifying['verified']

                        if result:
                            true_result_found = True  
                            image_dataset.loc[image_dataset["Image_Name"] == int(image.split('.')[0]), 'Crossing_Time'] = image_dataset.loc[image_dataset["Image_Name"] == int(image.split('.')[0]), 'Crossing_Time'].iloc[0] + 1
        ...
        if (confidence > 0.94) and (len(os.listdir("/home/ubuntu/Downloads/Telegram Desktop/image_temporary")) == 0):
